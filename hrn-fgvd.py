...
def train(epoches, net, trainloader, testloader, optimizer, scheduler, lr_adjt, dataset, CELoss, tree, device, devices, save_name):
    ...
    for epoch in range(epoches):
        ...
        if test_species_acc_soft > max_val_acc:
            ...
            net.cpu()
            # os.makedirs(w_dir + "/hrn_save_model/best/", exist_ok=True)
            # torch.save(net, w_dir + "/hrn_save_model/best/"+ save_name+ "_"+ epoch +'.pth')
...
def arg_parse():
    ...
    parser.add_argument('--dir', default=4, type=int, help='Working Directory')
...
if __name__ == '__main__':
    ...
    args = arg_parse()
    w_dir = args.dir
    model = w_dir + "/resnet50-19c8e357.pth"
    ...
    if dataset == 'FGVD':
        ...
        train_data_dir = w_dir + "/cropped/crop_train"
        test_data_dir = w_dir + "/cropped/crop_test"
        train_list = w_dir + "/cropped/train_list_new.txt"
        test_list = w_dir + "/cropped/test_list.txt"
    ...
    hrn_model_dict = torch.load("./hrn_best.pt")['state_dict']
...
#     test(net, testloader, CELoss, tree, device, 'FGVD')
    ...
    # pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv( w_dir + "/test_dumps_orig_list2_outl1.txt",header=False,index=False,sep='\t')
#     pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv( w_dir + "/test_dumps_orig_list2_outl2.txt",header=False,index=False,sep='\t')
#     pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv( w_dir + "/test_dumps_orig_list2_outl3.txt",header=False,index=False,sep='\t')
