...
def train(epoches, net, trainloader, testloader, optimizer, scheduler, lr_adjt, dataset, CELoss, tree, device, devices, save_name):
    ...
    for epoch in range(epoches):
        ...
        if test_species_acc_soft > max_val_acc:
            ...
            # torch.save(net, "/ssd_scratch/prafful/hrn_save_model/best/"+ save_name+ "_"+ epoch +'.pth')
    ...
    # torch.save(checkpoint, "/ssd_scratch/prafful/hrn_save_model/best/"+ save_name+ "_"+ best_epoch +'.pt')
...
def test(net, testloader, CELoss, tree, device, dataset):
    ...
    with torch.no_grad():
        ...
        idx = 0
        final_pred_conf_output_array = []
        final_pred_output_array = []
        imfilename_array = []
        for batch_idx, (inputs, targets, imfilenames) in enumerate(testloader):
            idx = batch_idx

            if(batch_idx==0):
                with open('sample_input_tensor.pkl', 'wb') as file:
                    pickle.dump(inputs.cpu(), file)
            inputs, targets = inputs.to(device), targets.to(device)
            order_targets, family_targets, target_list_sig = get_order_family_target(targets, device, dataset)
            ...
            #     print(order_targets.data)
            order_total += order_targets.size(0)
            order_correct += order_predicted.eq(order_targets.data).cpu().sum().item()
            
            # final_pred_conf_output_array = final_pred_conf_output_array+list(order_targets.data.cpu().numpy())
            # final_pred_output_array = final_pred_output_array+list(order_predicted.cpu().numpy())

            _, family_predicted = torch.max(xc2_sig.data, 1)
            # if(batch_idx==0):
            #     print(_)
            #     print(family_predicted)
            #     print(family_targets.data)
            family_total += family_targets.size(0)
            family_correct += family_predicted.eq(family_targets.data).cpu().sum().item()
            
#             final_pred_conf_output_array = final_pred_conf_output_array+list(family_targets.data.cpu().numpy())
#             final_pred_output_array = final_pred_output_array+list(family_predicted.cpu().numpy())
            ...
            # if(batch_idx==0):
            #     print(_)
            #     print(species_predicted_sig)
            #     print(select_leaf_labels.data)
            #     print(list(species_predicted_sig.cpu().numpy()))
                
            final_pred_conf_output_array = final_pred_conf_output_array+list(select_leaf_labels.data.cpu().numpy())
            final_pred_output_array = final_pred_output_array+list(species_predicted_soft.cpu().numpy())
            
            imfilename_array = imfilename_array+list(imfilenames)
            species_total += select_leaf_labels.size(0)
            species_correct_soft += species_predicted_soft.eq(select_leaf_labels.data).cpu().sum().item()
            species_correct_sig += species_predicted_sig.eq(select_leaf_labels.data).cpu().sum().item()
    ...
    return test_order_acc, test_family_acc, test_species_acc_soft, test_species_acc_sig, test_loss, final_pred_output_array, imfilename_array, final_pred_conf_output_array
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='PyTorch Deployment')
    parser.add_argument('--imgsdir', type=str, help='Dir path of cropped vehicle images')
    parser.add_argument('--imgslist', type=str, help='cropped vehicle images list txt')
    args = parser.parse_args()
    ...
    if dataset == 'FGVD':
        ...
        test_data_dir = args.imgsdir
        # test_data_dir = "/scratch/prafful/test_dumps_orig"
#         test_data_dir = "/scratch/prafful/orig_gt_object_images"
#         test_data_dir = "/scratch/prafful/test_dumps_orig_gtvalid"
        train_list = "./cropped/train_list_new.txt"
        # test_list = "./cropped/test_list.txt"
        test_list = args.imgslist
        # test_list = "/scratch/prafful/test_dumps_orig_list2.txt"
#         test_list = "/scratch/prafful/orig_gt_object_images_list2.txt"
#         test_list = "/scratch/prafful/test_dumps_orig_gtvalid_list2.txt"
        ...
        levels = 3
        total_nodes = 281
        # trainset = FGVDDataset(train_data_dir, train_list, transform_train, re_level='class', proportion=0.3)
        # Uncomment this line for testing OA results
        testset = FGVDDataset(test_data_dir, test_list, transform_test, re_level='class', proportion=0.3)
    ...
    # hrn_model_dict = torch.load("FGVD_100_448_p0.0_bz64_ResNet-50__Cos_90.pt", map_location=device)['state_dict']
    hrn_model_dict = torch.load("hrn_best.pt", map_location=device)['state_dict']
    hrn_model_dict_correct = {}
    for ki in hrn_model_dict.keys():
        hrn_model_dict_correct[ki.replace('module.','')] = hrn_model_dict[ki]
    net.load_state_dict(hrn_model_dict_correct)
    net.eval()
    ...
    # Evaluate OA
#     test(net, testloader, CELoss, tree, device, 'FGVD')
    _, _, _, _, _, final_pred_output_array, imfilename_array, final_pred_conf_output_array = test(net, testloader, CELoss, tree, device, 'FGVD')
    pdff = pd.read_csv(test_list,sep='\t',header=None)
    imnames = pdff[0].to_list()
    imfilename_array_new = [fi.split('/')[-1] for fi in imfilename_array]
    # print(len(imnames),len(final_pred_output_array),len(imfilename_array_new),len(final_pred_conf_output_array))
    
    fgvd_labels_list = list(FGV_labels.keys())
    pred_out_names = [fgvd_labels_list[fpi] for fpi in final_pred_output_array]
    hrntestoutput = pd.DataFrame({0:imfilename_array_new, 1:pred_out_names})
    hrntestoutput.to_csv(test_list[:-4]+"_hrnoutput.txt",header=False,index=False,sep='\t')
    
    print("###### HRN Output saved to --->",test_list[:-4]+"_hrnoutput.txt")
    # pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv(test_list[:-4]+"_hrnoutput.txt",header=False,index=False,sep='\t')
    
    # pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv("/home2/praffulkumar/cp_wks/yolov5/test_dumps_orig_list2_outl1.txt",header=False,index=False,sep='\t')
#     pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv("/home2/praffulkumar/cp_wks/yolov5/test_dumps_orig_list2_outl2.txt",header=False,index=False,sep='\t')
#     pd.DataFrame({0:imfilename_array_new, 1:final_pred_output_array, 2:final_pred_conf_output_array }).to_csv("/home2/praffulkumar/cp_wks/yolov5/test_dumps_orig_list2_outl3.txt",header=False,index=False,sep='\t')
